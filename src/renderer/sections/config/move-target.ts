...
export function resolveFamilyRoot(fam: string, knownPaths: string[]): string | undefined {
  const seg = MV_FAMILY_SEGMENTS[fam]
  if (!seg) return undefined
  const wantSegs = seg.toLowerCase().split('/')
  for (const raw of knownPaths) {
    const root = truncateAtFamily(raw, wantSegs, fam)
    if (root) return root
  }
  return undefined
}
...
function truncateAtFamily(raw: string, wantSegs: string[], fam: string): string | undefined {
  const segs = raw.replace(/\\/g, '/').split('/').filter(Boolean)
  const lc = segs.map((s) => s.toLowerCase())
  for (let i = 0; i + wantSegs.length <= lc.length; i++) {
    if (wantSegs.some((w, k) => lc[i + k] !== w)) continue
    // 'claude' (Einzel-`.claude`) darf nicht das Trunk-`.shared/.claude` treffen.
    if (fam === 'claude' && i > 0 && lc[i - 1] === '.shared') continue
    return segs.slice(0, i + wantSegs.length).join('/')
  }
  return undefined
}
...
export function buildQuickPath(
  ...
  const root = resolveFamilyRoot(fam, knownRoots)
  if (!root) return ''
  const base = root + '/' + cat + '/'
  return kind === 'Ordner' ? base + name + '/' : base + name
}

// Letztes Pfad-Segment (separator-robust /, \) — leer bei trailing Separator.
...
function hasFileExtension(seg: string): boolean {
  return /[^.\\/]\.[^.\\/]+$/.test(seg)
}
...
export function endsOnFolder(path: string, name: string, knownFolders: Set<string>): boolean {
  const p = path.trim()
  if (!p) return false
  if (/[\\/]$/.test(p)) return true
  if (knownFolders.has(p)) return true
  const seg = lastSegment(p)
  return seg !== name && !hasFileExtension(seg)
}
...
export function ensureFileTarget(path: string, name: string, kind: 'Datei' | 'Ordner', knownFolders: Set<string>): string {
  if (kind !== 'Datei') return path
  const p = path.trim()
  if (!p || !endsOnFolder(p, name, knownFolders)) return p
