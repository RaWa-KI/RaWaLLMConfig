...
import {
  ...
  REF_CAP,
  tokenDiagnostics
} from '../../src/renderer/sections/config/ConfigDiagnostics'
...
test('Diagnostics fuehren refs mit entryId statt reiner Namensliste', () => {
  ...
  const [d] = frontmatterDiagnostics(category([...hinted, clean]))
...
test('tokenDiagnostics behaelt den Token-Zusatz im Verweisnamen', () => {
  ...
  const [d] = tokenDiagnostics(category([heavy, entry({ id: 'l1', name: 'leicht' })]))
...
test('Kappung: maximal 4 Verweise sichtbar, Rest als hiddenCount („+ n weitere")', () => {
  ...
  const capped = capRefs(refs)
  expect(REF_CAP).toBe(4)
  ...
  expect(capRefs(refs.slice(0, 3))).toEqual({ shown: refs.slice(0, 3), hiddenCount: 0 })
