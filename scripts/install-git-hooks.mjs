// Installiert die versionierten Hooks (pre-commit, pre-push) nach .git/hooks (WP24).
...
for (const hook of HOOKS) {
  ...
  if (existsSync(dst)) {
    ...
    console.log(`hooks:install: vorhandene ${hook}-Hook gesichert nach ${bak}`)
  }
  ...
  console.log(`hooks:install: ${hook} installiert.`)
}
